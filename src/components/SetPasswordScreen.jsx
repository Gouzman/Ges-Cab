...
const SetPasswordScreen = ({ user, onPasswordSet }) => {
  ...
  const handleSetPassword = (e) => {
    ...
    const members = JSON.parse(localStorage.getItem('legalTeamMembers') || '[]');
    const updatedMembers = members.map(member => {
      if (member.id === user.id) {
        return { ...member, password: password, firstLogin: false };
      }
      return member;
    });
    localStorage.setItem('legalTeamMembers', JSON.stringify(updatedMembers));

    const updatedUser = { ...user, password: password, firstLogin: false };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 p-4">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400">Veuillez définir votre mot de passe personnel.</p>
        </div>
        ...
        <form onSubmit={handleSetPassword} className="space-y-6">
          ...
          <Button
            type="submit"
            className="w-full h-12 text-lg bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700"
          >
